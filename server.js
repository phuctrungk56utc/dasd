...
socketIo.on("connection", (socket) => { ///Handle khi có connect từ client tới
  ...
  socket.on("sendDataClient", function (data) { // Handle khi có sự kiện tên là sendDataClient từ phía client
    ...
    user.id = socket.id;
    let obj = listUSer.find(o => o.id === socket.id);
    if(!obj){
      listUSer.push(user);
    }
...
let port = 8017;
server.listen(3001, () => {
  console.log(`Hello trungtp, I'm running at localhost:${port}/`);
});
app.listen(8017, () => {
  console.log(`Hello trungtp, I'm running/`);
});
module.exports.ioObject = {socketIo:socketIo,listUSer:listUSer};
