 ...
 * @param {*} res 
 */
let postUserRole = async (req, res) => {
    try {
        var queryRoot = `DELETE FROM prm."userRole" WHERE "userId" IN (`;
        ...
        if(req.body.params.params.data.length > 0){
            var queryInsert = `INSERT INTO prm."userRole"(
                "RoleID", "userId")
                VALUES`;
                ...
                for (let i in req.body.params.params.data) {
                    var element = ''
                    if (lengthList === Number(i) + 1) {
                        element = `('${req.body.params.params.data[i].RoleID}','${req.body.params.params.data[i].userId}')`;
                    } else {
                        element = `('${req.body.params.params.data[i].RoleID}','${req.body.params.params.data[i].userId}'),`;
                    }
                    queryInsert += element;
